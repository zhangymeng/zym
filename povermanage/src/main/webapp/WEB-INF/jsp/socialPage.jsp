...
<table class="layui-table" lay-data="{width: 1300, height:500, url:'<%=basePath%>/social/findAll', page:true, id:'idTest'}" lay-filter="demo">
  <thead>
    <tr>
...
layui.use(['table', 'form'],function(){
  ...
  table.on('tool(demo)', function(obj){
    ...
    if(obj.event === 'detail'){
      ...
      window.location.href = "<%=basePath%>social/addSDPage?sId="+data.id;
    } else if(obj.event === 'del'){
    //删除操作
      layer.confirm('真的删除行么', function(index){
      $.ajax({
		url: "<%=basePath%>/social/delSocial",
                    data: {
...
form.on('submit(demo1)', function(data){
  ...
  var $ = layui.$, active = {
  	//添加
    add: function(){ 
		window.location.href = "<%=basePath%>/social/addPage";
    }
